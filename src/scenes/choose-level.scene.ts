            ...
            for (let j = 0; j < 6; j++) {
                ...
                if (nr <= LevelService.getMaxAvailableLevel(data.difficulty)) {
                    ...
                    if (LevelService.getSolvedLevels(data.difficulty).indexOf(nr) !== -1) {
                        sprite = this.add.sprite(j * distance + offsetX, i * distance + offsetY, 'rectangle_green').setOrigin(0.5, 0.5).setInteractive();
                    } else {
                        sprite = this.add.sprite(j * distance + offsetX, i * distance + offsetY, 'rectangle').setOrigin(0.5, 0.5).setInteractive();
                    }
                    ...
                    this.add.sprite(j * distance + offsetX, i * distance + offsetY, 'rectangle').setOrigin(0.5, 0.5).setInteractive();
                    tint = 0x999999;
                }
                this.add.text(j * distance + offsetX, i * distance + offsetY, nr.toString(), {font: '24px'}).setOrigin(0.5, 0.5).setTint(tint);
            }
        }
